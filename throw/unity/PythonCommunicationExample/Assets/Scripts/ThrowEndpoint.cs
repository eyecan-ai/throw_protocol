...
public class ThrowEndpoint<K> : MonoBehaviour
{
    ...
    */
    public List<MessageCallback<K>> passive_callbacks = new List<MessageCallback<K>>();
    public MessageCallbackWithResponse<K> active_callback = null;
    ...
    */
    void ConnectionLoop(ThrowNode client)
    {
        ...
        while (true)
        {
            ...
            {
                // Receive message
                Message<K> message = client.receiveMessage<K>();

                // Produce Response message if active callback is set
                Message<K> response_message = new Message<K>();
                ...
                foreach (MessageCallback<K> callback in passive_callbacks)
                {
                    callback(message);
                }
