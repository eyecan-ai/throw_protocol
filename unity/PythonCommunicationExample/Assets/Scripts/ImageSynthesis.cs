...
public class ImageSynthesis : MonoBehaviour
{
    ...
    private CapturePass[] capturePasses = new CapturePass[] {
        new CapturePass() { name = "_img", enabled=true },
        new CapturePass() { name = "_id", supportsAntialiasing = false, enabled = false },
        new CapturePass() { name = "_layer", supportsAntialiasing = false , enabled = true},
        new CapturePass() { name = "_depth", enabled=true },
        new CapturePass() { name = "_normals", enabled = false },
        new CapturePass() { name = "_flow", supportsAntialiasing = false, needsRescale = true, enabled = false } // (see issue with Motion Vectors in @KNOWN ISSUES)
	};
    ...
    private void Save(string filenameWithoutExtension, string filenameExtension, int width, int height)
    {
        ...
        {
            if (pass.enabled)
                Save(pass.camera, filenameWithoutExtension + pass.name + filenameExtension, width, height, pass.supportsAntialiasing, pass.needsRescale, pass);
        }
    }

    private void Save(Camera cam, string filename, int width, int height, bool supportsAntialiasing, bool needsRescale, CapturePass pass)
    {
        var mainCamera = GetComponent<Camera>();
        var depth = 24;
        ...
        lock (locker)
        {
            if (string.Compare(pass.name, "_depth") == 0)
            {
